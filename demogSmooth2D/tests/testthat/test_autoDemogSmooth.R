...
test_that("Testing autoDemogSmooth on linear data with noise, effects = FALSE", {
  sm = autoDemogSmooth(m + err, effects = FALSE)
  expect_equivalent(sm$result, m)
  expect_null(sm$yearsEffect)
  expect_null(sm$cohortEffect)
})

test_that("Testing autoDemogSmooth on linear data with noise, effects = TRUE", {
  sm = autoDemogSmooth(m + err, effects = TRUE)
  expect_equivalent(sm$result, m)
